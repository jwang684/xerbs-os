...
- The active organization is resolved in exactly one place — `getAuthContext`.
  Today it comes from the user's membership (with an optional `x-organization-id`
  header to disambiguate multi-org users). Moving org selection fully into the
  session later requires changing only that function; services never see how the
  org was chosen.
...
- Reads are allowed for any organization member; writes require role `owner`,
  `admin`, or `practitioner` (`staff` is read-only), enforced by `assertCanWrite`.
